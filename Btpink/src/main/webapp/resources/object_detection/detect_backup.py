...
def load_image_into_numpy_array(image):
  ...
  return np.array(image.getdata()).reshape(
      (im_height*1, im_width*1, 3)).astype(np.uint8)
...
IMAGE_PATHS = os.path.join(PATH_TO_TEST_IMAGES_DIR, '111.jpg')
print(IMAGE_PATHS)
# 출력 이미지 사이지(inche 단위).
IMAGE_SIZE = (24, 16)
cnt = 0
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
      ...
      for i in range(num):
          if (np.squeeze(scores)[i] >= 0.5 and np.squeeze(classes).astype(np.int32)[i] == 1):
              cnt = cnt + 1
